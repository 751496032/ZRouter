...
给NavDestination设置路由转场动画

设置全局转场动画
```typescript

// 1、在主页面设置转场动画回调 并 设置全设置全局转场动画
@Entry
@Component
struct Index {
  ...
  build() {
    Navigation(ZRouter.getNavStack())
      .customNavContentTransition(ZRouter.getInstance()
        ...
        .setPushEnterAnimate({
          ...
          startCallback: (m) => {
            ...
            // 缩放
            m.attribute?.scale({ x: 0, y: 0})
            // 透明度
            m.attribute?.opacity(0.01)
            // 旋转
            m.attribute?.rotate({ angle: 90 })
            // 等等
          },
          // 动画结束的回调
          endCallback: (m) => {
            ...
            // 缩放
            m.attribute?.scale({ x: 1, y: 1})
            // 透明度
            m.attribute?.opacity(1)
            // 旋转
            m.attribute?.rotate({ angle: 0 })
            // 等等
          },
        })
          // 退出状态(POP) 离开页的动画转场参数（B返回A 的 B离开转场）
        .setPopExitAnimate({
          startCallback: (m) => m.attribute?.translate({ y: '0' }),
          endCallback: (m) => m.attribute?.translate({ y: '100%' }),
        })
...
// 2、设置自定义转场
// 只需要设置startCallback
// 记得动画结束手动调用proxy?.finishTransition()结束转场
@Route({ name: "Demo" })
@Component
export struct Demo {
  // 定义NavAnimationModifier
